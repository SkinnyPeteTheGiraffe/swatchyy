...
class Template
{
    ...
    public function render(array $context = [])
    {
        if ($template = locate_template($path = $this->getRelativePath(), false, false)) {
            $this->doActions();
            extract(apply_filters("matik/swatchyy/template/context/{$this->getFilename()}", $context));
            require $template;
            return;
        }
    ...
    public function getFilename($extension = '.php')
    {
        ...
        return apply_filters('matik/swatchyy/template/filename', "{$this->file}{$extension}");
    }
    /**
